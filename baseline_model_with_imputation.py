...
SUBMISSION_OUTPUT_FILE = f'{DATA_DIR}submission_baseline_v4_impute_rounded.csv'

# --- Setup Logging ---
log_file = 'Metrics.log'
# Use a specific name for this run configuration
model_name = "LGBM_MultiOutput_RandomSearch_v4_impute_rounded"
...
# Separate features (X) and target (y) - NOW INCLUDES ROWS THAT MAY HAVE NaNs in X
X_train = train_full_df[METADATA_COLS + WEEK1_FEATURE_COLS].copy()
# --- IMPORTANT: Check if target (y_train) has NaNs. If so, those rows MUST be dropped ---
# ---             as we cannot train without a valid target value.              ---
...
valid_target_indices = y_train.dropna().index
initial_rows = len(X_train)
X_train = X_train.loc[valid_target_indices]
y_train = y_train.loc[valid_target_indices]
time_train = time_train.loc[valid_target_indices]
...
# Log data shape *before* imputation (reflects data fed into pipeline)
logger.info(f"Training data shape before imputation (target NaNs dropped): X_train={X_train.shape}, y_train={y_train.shape}")

print(f"Training data shapes before imputation: X_train={X_train.shape}, y_train={y_train.shape}")
...
print("Setting up preprocessing pipeline (WITH Imputation)...")
...
categorical_transformer = Pipeline(steps=[
    ('imputer', SimpleImputer(strategy='most_frequent')), # <<<--- ADD IMPUTER (or strategy='constant', fill_value='missing')
    ('onehot', OneHotEncoder(handle_unknown='ignore', sparse_output=False))
])

ordinal_transformer = Pipeline(steps=[
    ('imputer', SimpleImputer(strategy='most_frequent')), # <<<--- ADD IMPUTER (imputes before encoding)
    ('ordinal', OrdinalEncoder(categories=[ais_categories], # List of lists
                               handle_unknown='use_encoded_value',
                               unknown_value=-1)), # Assign -1 to unknowns (handles potential post-imputation issues if needed)
    # ('scaler', StandardScaler()) # Optional scaler after encoding
])

# Use ColumnTransformer (definition remains the same, points to updated transformers)
...
print("\nPipeline V4 (Impute, Ordinal AIS, Rounded) finished.")
print("Next steps:")
